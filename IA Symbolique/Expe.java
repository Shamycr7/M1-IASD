...
public class Expe {
	...
	public static void main(String[] args) throws Exception{
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter("courbe.csv"));
		writer.write("durté" + ";" +  "pourcentage" + "\n");
		
		
		int i =178;
		while(i<=211) {
			System.out.println("fichir csp" + i+ ".txt \n");
		...
		for(int nb=1 ; nb<=nbRes; nb++) {
			...
			if(model==null) {
				...
				if(model.getSolver().solve()) {
					System.out.println("nbrSolution  : "+ model.getSolver().getSolutionCount()+"\n");
					resavecsol ++;
				}	
		...
		int  pourcentage = (resavecsol*100)/nbRes;
		
		System.out.println("nombre de reseau ayant une solution :"+resavecsol);
		
		System.out.println("pourcentage:"+pourcentage +"\n");
		System.out.println("*************************************\n");
		
		double d = (((17*17) - (double)(i)) / (17*17));
		String durte= Double.toString(d);
		String pourc= Integer.toString(pourcentage);
		writer.write(d + ";" +  pourc + "\n");
