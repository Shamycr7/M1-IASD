...
public class WolvesApp extends JFrame {
	...
	public WolvesApp(String title, int numbrows, int numbcols, int squaresize, Wolves game) {
		...
		//game = new Wolves(numbrows, numbcols,5,5,5,1,2);
		this.game = game;
		
		//System.out.printf("WolvesApp : NumPreys = %d%n",game.getNumPreys());
		//System.out.printf("WolvesApp : NumWolves = %d%n",game.getNumWolves());
		//System.out.println();
		
		//WolvesUI panel = new WolvesUI(game,squaresize);
		WolvesUI panel = new WolvesUI(game,squaresize, game.getStrategy());
		...
		pauseButton.addMouseListener(new MouseListener() {
			public void mouseReleased(MouseEvent e) {
				if (pauseButton.getText().equals("Pause")) {
					...
					while(true) {
						/*
						while(res < Math.pow(2,20)) {
							res++;
						}
						if(res == Math.pow(2,20)) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (1 >= delay) throw new Exception();
		}	
		catch(Exception e) {
			//e.printStackTrace();
			System.err.println("");
		...
		//Wolves game = new Wolves(height,width,0,0,10,1,2);
		//WolvesApp wol = new WolvesApp("Hungry Hungry Wolves", height, width, squaresize,game);
		//wol.runGoL(delay);
		System.err.println("Lancez Essai.java");
	}
